...
![Decoder pipeline](shaclWorkflowImage.jpg)
...
flowchart TD
    ...
    S1["1 · decoder-extract-metadata<br/>fetch URL · reuse embedded JSON-LD<br/>else LLM extract"]
    S1 -->|"01_extracted.json"| S2

    S2["2 · decoder-lift-rdf<br/>deterministic schema:Dataset Turtle"]
    S2 -->|"02_graph.ttl"| S3

    S3["3 · decoder-validate-shacl<br/>pySHACL + googleRecommended.ttl"]
    S3 -->|"03_results.json · 03_conforms.json"| S4

    S4["4 · decoder-report-findings<br/>fixType / autoFixable + issue prose"]
    S4 -->|"04_report.json"| DEC

    DEC{"decide<br/>conforms? · auto-fixable left?<br/>progress? · max iterations?"}

    DEC -->|"repair"| S5
    S5["5 · decoder-repair-graph<br/>add from source · coerce · remove · reword"]
    S5 -->|"05_graph.ttl"| S3

    DEC -->|"done"| S6
    S6["6 · decoder-emit-provenance<br/>RAiD record + narrative"]
    S6 --> OUT["06_raid.json · 06_record.md"]

    classDef skill fill:#1e3a5f,stroke:#4a9eff,color:#e8f1ff
    classDef io fill:#2a2a2a,stroke:#888,color:#ddd
    classDef decide fill:#3d2f1e,stroke:#e0a040,color:#fff4e0
...
|---|-------|--------------|:----:|
| 1 | **decoder-extract-metadata** | Fetch the URL; reuse embedded schema.org JSON-LD, else LLM-extract `url/name/description/keywords` | fallback |
| 2 | **decoder-lift-rdf** | Deterministically lift to a `schema:Dataset` Turtle graph | no |
| 3 | **decoder-validate-shacl** | Validate with pySHACL against the Google Dataset shape | no |
| 4 | **decoder-report-findings** | Tag each finding with a deterministic `fixType`; enrich prose | optional |
| 5 | **decoder-repair-graph** | Fix from source / coerce / remove / LLM-reword; loop back to 3 | optional |
| 6 | **decoder-emit-provenance** | Write a RAiD-style provenance record + narrative | optional |
...
SHACL_bundle/
  ...
  README.md                     # this file
  decoder-extract-metadata/      SKILL.md  assets/extract.py  schema.json  extract_prompt.md
  decoder-lift-rdf/ SKILL.md  assets/lift.py  jsonld_context.json  examples/
  decoder-validate-shacl/ SKILL.md  assets/validate.py  googleRecommended.ttl
  decoder-report-findings/ SKILL.md  assets/parse_report.py  violation_schema.json
  decoder-repair-graph/          SKILL.md  assets/repair.py  reprompt_template.md
  decoder-emit-provenance/  SKILL.md  assets/render_record.py  raid_template.json  narrative_prompt.md
...
stderr (`→ stage 3/6 decoder-validate-shacl: run pySHACL …`) so a slow stage — most
...
```bash
RUN=runs/dev
python decoder-extract-metadata/assets/extract.py        "<url>"            --out-dir $RUN
python decoder-lift-rdf/assets/lift.py   $RUN/01_extracted.json --out-dir $RUN
python decoder-validate-shacl/assets/validate.py  $RUN/02_graph.ttl  --out-dir $RUN
python decoder-report-findings/assets/parse_report.py $RUN/03_results.json --out-dir $RUN
python decoder-repair-graph/assets/repair.py              $RUN/02_graph.ttl $RUN/04_report.json --out-dir $RUN
python decoder-emit-provenance/assets/render_record.py $RUN
```
...
`decoder-validate-shacl/assets/googleRecommended.ttl` targets `schema:Dataset` (all
