...
def stage1(state: PipelineState) -> dict:
    _log(state, "→ stage 1/6 llm-output: fetch URL + extract metadata …")
...
def stage2(state: PipelineState) -> dict:
    _log(state, "→ stage 2/6 rdf-knowledge-graph: lift to RDF …")
...
def stage3(state: PipelineState) -> dict:
    _log(state, "→ stage 3/6 shacl-validation: run pySHACL (RDFS inference) …")
...
def stage4(state: PipelineState) -> dict:
    _log(state, "→ stage 4/6 violation-report: build fix-oriented report …")
...
def stage5(state: PipelineState) -> dict:
    ...
    _log(state, f"→ stage 5/6 repair (pass {it}): apply fixes …")
...
def stage6(state: PipelineState) -> dict:
    _log(state, "→ stage 6/6 trusted-output: write provenance record …")
