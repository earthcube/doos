...
uv pip install -r skills/bco-dmo-scan/assets/requirements.txt
```
...
written to `skills/bco-dmo-scan/runs/<UTC-timestamp>/`.

```bash
# Keyword search (default search term is "depth" if neither --search nor --catalog is set)
python skills/bco-dmo-scan/assets/run_pipeline.py --search depth

# Custom output directory
python skills/bco-dmo-scan/assets/run_pipeline.py \
  --search "dissolved oxygen depth" \
  --work-dir skills/bco-dmo-scan/runs/oxygen-test

# Smoke test — first 5 datasets only
python skills/bco-dmo-scan/assets/run_pipeline.py --search depth --limit 5

# Full ERDDAP catalog (~2,500 datasets) instead of keyword search
python skills/bco-dmo-scan/assets/run_pipeline.py --catalog --work-dir /tmp/bco-full
```
...
`skills/bco-dmo-scan/output/output.nt`. After a pipeline run:

```bash
cp skills/bco-dmo-scan/runs/<timestamp>/output.nt skills/bco-dmo-scan/output/output.nt
```
...
| `--work-dir PATH` | Output root (default: `skills/bco-dmo-scan/runs/<timestamp>`) |
...
```bash
python skills/bco-dmo-scan/assets/main.py --help
python skills/bco-dmo-scan/assets/main.py erddap --help
python skills/bco-dmo-scan/assets/main.py iso --help
```

### Stage 1 — ERDDAP inventory

```bash
python skills/bco-dmo-scan/assets/main.py erddap \
  --search depth \
...
```bash
python skills/bco-dmo-scan/assets/main.py iso \
...
summarize results from `run.json`. Invoke via `/bco-dmo-scan` in supported
