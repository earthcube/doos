        ...
        for row in reader:
            ...
            if level == 0:
                ...
                pattern_triples.append(
                    f"?{get_var_name(top_type)} <{prop}> ?{var_name}"
                )

                prop_name = get_var_name(prop)
                top_type_name = get_var_name(top_type)
                description = (
                    f"Triple pattern to get the '{prop_name}' from a '{top_type_name}'."
                )
            ...
            if pattern_triples:
                results.append({"pattern": pattern_triples, "description": description})
...
def main():
    parser = argparse.ArgumentParser(
        description="Process a CSV file to generate JSON with SPARQL patterns."
    )
