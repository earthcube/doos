                ...
                for script in jsonld_scripts:
                    try:
                        ...
                        if isinstance(jsonld, dict):
                            print(f"Warning: @type is '{jsonld.get('@type', 'unknown')}', expected 'Dataset'", file=sys.stderr)
                            return jsonld
                    except json.JSONDecodeError:
                        continue
...
def extract_distributions(jsonld):
    ...
    for dist in distributions:
        ...
        is_supported = (
            encoding_format in [f.lower() for f in supported_formats] or
            content_url.lower().endswith('.csv') or
            content_url.lower().endswith('.parquet')
        )
...
def main():
    ...
    try:
        ...
        for col_name, stats in depth_results.items():
            ...
            print(f"  Standard deviation: {stats['std_dev']:.2f} meters", file=sys.stderr)
