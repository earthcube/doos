...
def print_info(parquet_file):
    ...
    if all(col in gdf.columns for col in selected_cols):
        ...
        print(
            f"One or more columns not found: {missing}. Full columns: {list(gdf.columns)}"
        )
...
def rml_mapping(parquet_file, template):
    ...
    if all(col in gdf.columns for col in selected_cols):
        ...
        for index, row in df.iterrows():
            ...
            json_ld_doc["geosparql:hasGeometry"]["geosparql:asWKT"]["@value"] = row[
                "geometry"
            ]
