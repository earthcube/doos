#!/usr/bin/env python
"""
Stage 6 — Trusted Output (SHACL-for-AI-outputs flow).
...
def _deterministic_narrative(facts: dict) -> str:
    ...
    parts = [
        f"The dataset at {src} was processed by the shacl-for-ai-outputs pipeline."
    ]
    parts.append(
...
def build_raid(facts: dict, run_id: str, start: str, end: str) -> dict:
    ...
    raid["contributor"][0]["_note"] = (
        "Automated pipeline agent: DOOS shacl-for-ai-outputs."
    )
